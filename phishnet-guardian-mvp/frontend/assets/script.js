...
function getSelectedLangFrom(selectorId) {
  const el = document.getElementById(selectorId);
  if (el && el.value) return el.value;              // supports 'hi-IN', 'mr-IN', 'en', 'auto'
  return navigator.language || "auto";              // fallback to browser locale
}

function normalizeLangTag(tag) {
  // Keep whatever the user selected; backend normalizes to base 'en|hi|mr' or 'auto'
  const t = (tag || "").trim();
...
function renderQuiz(container, quiz) {
  ...
  container.innerHTML = `
    <div class="quiz-wrap">
      ...
      <div class="quiz-opts">
        ${options
          .map((o) => `<button class="quiz-opt outline" data-val="${o}">${o}</button>`)
          .join("")}
      </div>
...
function renderResult(r) {
  ...
  // Debug badge to see which language the backend used (optional)
  if (r._lang_used) {
    const badge = document.createElement("div");
    badge.style.fontSize = "12px";
    badge.style.opacity = "0.7";
    badge.style.marginTop = "4px";
    badge.textContent = `Language used: ${r._lang_used}`;
...
// ---------------- API HELPERS ----------------
// NOTE: send both 'language' in body (text) and 'Accept-Language' header.
// For images, add '?lang=' in the query string and include the header as well.

async function scanText({ text, language }) {
  const lang = normalizeLangTag(language);
...
async function scanImage(file, language) {
  const lang = normalizeLangTag(language);

  // Use query param for FastAPI and include header (backend supports both)
  const res = await fetch(`${API}/api/scan-image?lang=${encodeURIComponent(lang)}`, {
    method: "POST",
    headers: {
      "Accept-Language": lang
    ...
    body: (() => {
      ...
      form.append("file", file);            // MUST be named "file" for FastAPI UploadFile
      // Including both doesn't hurt older code paths:
...
async function onAnalyze(ev) {
  ...
  const lang = normalizeLangTag(getSelectedLangFrom("language")); // 'language' select
  ...
  try {
    const data = await scanText({ text, language: lang });
    renderResult(data);
    await loadHistory();
    await loadStats();
  } catch (e) {
...
async function onAnalyzeImage(ev) {
  ...
  const lang = normalizeLangTag(getSelectedLangFrom("imgLanguage")); // 'imgLanguage' select
